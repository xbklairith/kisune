...
**Keep:** Product names, framework names, team-owned component names, JIRA keys, PR numbers, customer/workload identifiers. These are the cross-reference bridge — losing them breaks tracking.
...
4. JIRA back-post: only if the user explicitly says so. Show the payload, wait for explicit *"post it"* / *"go ahead"*, then post.
5. **Never auto-post to Slack, email, or any non-JIRA channel.** Hand the draft to the user; they post it.
...
- **Never strip a JIRA key, PR number, or customer/workload name** during de-jargoning.
